...
function form() {
  const questionOne = [{
    type: 'input',
    name: 'pageName',
    message: 'input page name which contains a form(eg. myPage):',
  }];
  const questionTwo = [{
    type: 'input',
    name: 'formName',
    message: 'input form name which contains a form(eg. myForm):',
  }];
        ...
        fs.access(targetPagePath, function (err) {
          ...
          inquirer.prompt(questionTwo)
            .then(function (answerTwo) {
              upperFormName = firstUpperFunction(answerTwo.formName);
              lowerFormName = firstLowerFunction(answerTwo.formName);

              setTargetPath(async function(targetCom) {

                copyComPath = targetCom;
                targetComponentPath = path.join(copyComPath,"/",upperFormName);
                ...
                fs.access(targetComponentPath, async function (err) {
                  ...
                  await copyDir(path.join(boilerplateDir, "./form/BoiPage"),copyPagePath);
                  ...
                  pageData = pageData.replace(/\${upperFormName}/g, upperFormName);
                  ...
                  await copyDir(path.join(boilerplateDir, "./form/component/MyForm"),copyComPath);
                  await renameDir(path.join(copyComPath, "/MyForm"), targetComponentPath);
                  var comPathTmp = path.join(targetComponentPath, "/index.js");
                  var formData = fs.readFileSync(comPathTmp,"utf-8");
                  formData = formData.replace(/\${lowerFormName}/g, lowerFormName);
                  formData = formData.replace(/\${upperFormName}/g, upperFormName);
                  try {
                    fs.writeFileSync(comPathTmp, formData, 'utf8');
                    console.log(chalk.green('generated form package success'));
                  } catch (e) {
                    console.log(chalk.red(`generated form package fail`, e));
                  }
                  ...
                  setTargetPath(async function (targetModel) {
                    ...
                    var modelPathTmp = `${copyModelPath}${lowerFormName}.js`;

                    await copyDir(
                      path.join(boilerplateDir, "./form/model/MyForm.js"),
                      modelPathTmp
                    );
                    var modelData = fs.readFileSync(modelPathTmp,"utf-8");
                    modelData = modelData.replace(/\${lowerFormName}/g, lowerFormName);
                    try {
                    ...
                    setTargetPath(async function (targetRoute) {
                      ...
                      var routeData = fs.readFileSync(path.join(boilerplateDir, "./form/route/BoiRoute.js"),"utf-8");
                      routeData = routeData.replace(/\${lowerPageName}/g, lowerPageName);
                      routeData = routeData.replace(/\${upperPageName}/g, upperPageName);
                      routeData = routeData.replace(/\${lowerFormName}/g, lowerFormName);
...
function table() {
  ...
  const questionTwo = [{
    type: 'input',
    name: 'tableName',
    message: 'input table name which contains a table(eg. myTable):',
  }];
